...
function updateFontPreviews(){['description','price'].forEach(type=>{const family=document.getElementById(`${type}-font`).value;document.getElementById(`${type}-font-preview`).style.fontFamily=`"${family}", cursive`})}
updateFontPreviews();
const template = new Image();
const templateDefinitions = {
  three:{name:'3 Photo Gecko Post',src:'assets/canonical-master.jpg',slots:[{x:94,y:93,w:532,h:283},{x:94,y:485,w:532,h:283},{x:94,y:879,w:532,h:282}],descriptionY:422,priceY:836},
...
function controls(){
  ...
  photos.forEach((photo,index)=>{
    const box=document.createElement('section');box.className='photo-compact';
    box.innerHTML=`<label class="zoom-control" aria-label="Photo ${index+1} zoom"><span>−</span><input type="range" min="100" max="140" step="1" value="100"><span>+</span><output>100</output></label>${index===1?'<div class="middle-order"><button type="button" class="order-button" data-slot="1" data-direction="up" aria-label="Swap middle photo upward">↑</button><button type="button" class="order-button" data-slot="1" data-direction="down" aria-label="Swap middle photo downward">↓</button></div>':''}`;
    const input=box.querySelector('.zoom-control input'),output=box.querySelector('.zoom-control output');
    input.addEventListener('input',()=>{output.textContent=input.value;photo.zoom=Number(input.value)/100;draw();queueAutosave()});
    photo.controls.zoom={input,output};
    ['x','y'].forEach(key=>photo.controls[key]={input:{value:0},output:{textContent:0}});
    host.append(box);
  });
}
controls();
...
function selectTemplate(){
  ...
  document.getElementById('upload-photos').textContent=`Upload ${slots.length} Photo${slots.length===1?'':'s'}`;
  document.querySelectorAll('#photo-controls .photo-compact').forEach((section,index)=>section.hidden=index>=slots.length);
  document.querySelectorAll('#photo-controls .photo-compact').forEach((section,index)=>{if(index<slots.length)section.style.top=`${((slots[index].y+slots[index].h/2)/12.8).toFixed(2)}%`});
...
const groupPhotoInput=document.getElementById('photo-files');
document.getElementById('upload-photos').addEventListener('click',()=>groupPhotoInput.click());
groupPhotoInput.addEventListener('change',()=>{
  const selected=[...(groupPhotoInput.files||[])].slice(0,slots.length);
  selected.forEach((file,index)=>loadPhotoFile(file,index,null));
  groupPhotoInput.value='';
  if(selected.length<slots.length)status.textContent=`Loaded ${selected.length} photo${selected.length===1?'':'s'}. Choose ${slots.length} together to replace the full set.`;
});
...
document.getElementById('custom-font-file').addEventListener('change',async event=>{
  ...
  try{
    ...
    document.getElementById('description-font').value=family;document.getElementById('font-status').textContent=`Loaded: ${selected.name}`;updateFontPreviews();draw();
  }catch(error){document.getElementById('font-status').textContent='Font could not be loaded.'}
});

['description','price','description-font','price-font'].forEach(id=>document.getElementById(id).addEventListener('input',()=>{updateFontPreviews();draw();queueAutosave()}));
...
document.getElementById('preview-action').addEventListener('click',()=>exportButton.click());
...
selectTemplate();[template,...photos.map(p=>p.image)].forEach(image=>image.addEventListener('load',draw));restoreDraft();
